...
class HceHostApiImpl: NSObject, FlutterPlugin, HceHostApi {
    ...
    func exposeMessage(message: String, completion: @escaping (Result<Void, Error>) -> Void) {
        ...
        let ProcessApdu: (_: Data) -> Data = {
            _ in self.getMessageApdu(message: message)
        }
                ...
                completion(
                    .failure(
                        PigeonError(
                            code: "NOT_AVAILABLE",
                            message:
                                "CardSession is not available, supported or the device is not eligible.",
                            details: nil)))
                return
            }
            ...
            do {
                ...
                completion(
                    .failure(
                        PigeonError(
                            code: "STARTUP_FAIL",
                            message: "Failed to acquire presentmentIntent or CardSession",
                            details: nil)))
                return
            }
            ...
            for try await event in cardSession.eventStream {
                ...
                case .readerDetected:
                    do {
                        ...
                        print("got error while starting emulation")
                        print(error)
                    }
            ...
            if !hasSent {
                completion(
                    .failure(
                        PigeonError(
                            code: "DID_NOT_SEND",
                            message: "For some reason, no event caused sending a response :(",
                            details: nil)))
            }
